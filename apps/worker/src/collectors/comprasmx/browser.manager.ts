...
export class BrowserManager {
  ...
  async launch(): Promise<void> {
    ...
    log.info({ headless: config.PLAYWRIGHT_HEADLESS }, '🚀 Launching Chromium browser...');
  ...
  async createContext(): Promise<BrowserContext> {
    ...
    log.info('🔄 Creating isolated browser context...');
    
    // Si ya había contexto, lo matamos (para evitar filtración de Memoria/Cookies)
    if (this.context) {
    ...
    if (config.PROXY_ENABLED && (config.HTTP_PROXY || config.HTTPS_PROXY)) {
      proxy = { server: config.HTTP_PROXY || config.HTTPS_PROXY || '' };
      log.info({ server: proxy.server }, '🌐 Proxy enabled via ENV');
    }
    ...
    // Definición global de timeout (30s) para evitar que queries lentas bloqueen infinito
