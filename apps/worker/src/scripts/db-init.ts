...
async function run() {
  ...
  if (!dbUrl) {
    ...
    console.error(
      "Este script requiere conexión directa a Postgres para poder ejecutar CREATE TABLE.",
    );
    console.error(
      "Configúrala en .env. (ej. postgresql://postgres.xxxx:password@aws-0-....)",
    );
    process.exit(1);
  }
  ...
  if (!fs.existsSync(schemaPath)) {
    console.error(
      `💥 ERROR: No se encontró el archivo de schema en: ${schemaPath}`,
    );
    process.exit(1);
  }
