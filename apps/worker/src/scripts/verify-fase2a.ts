...
async function main() {
  ...
  if (!dbUrl) {
    log.fatal('❌ SUPABASE_DB_URL no está configurado. Requerido para verificación directa de schema.');
    process.exit(1);
  }
  ...
  try {
    ...
    const expectedColumns = ['lightweight_fingerprint', 'last_detail_checked_at', 'last_attachments_checked_at'];
    ...
    if (allColumnsPass && indexPass) {
      log.info('🎉 Verificación Fase 2A EXITOSA. Schema preparado para entorno incremental.');
      process.exit(0);
    } else {
      log.error('💥 Fallo en verificación de Fase 2A. Revisa la salida e intenta correr la migración.');
      process.exit(1);
    }
